<template>
    <v-app class="font-weight-light">
        <v-main>
            <v-container>
                <v-card class="mt-5 mx-5" min-height="500px" color="indigo lighten-1">
                    <v-row>
                        <v-col cols="4" align="center">
                            <v-avatar class="ma-5" size="200" rounded="10">
                        ...
                        <v-col cols="8">
                            <v-row>
                                <v-card-text>
                                    <v-card-title>
                                        <v-row>
                                            <v-col align="center">
                                                <span class="font-weight-bold">{{ followCounter.notebooks }}</span>
                                                <br> Notebooks
                                            </v-col>
                                            <v-col align="center">
                                                <span class="font-weight-bold" @click="followedBy">{{ followCounter.followed
                                                }}</span>
                                                <br> Followers
                                            </v-col>
                                            <v-col align="center">
                                                <span class="font-weight-bold" @click="following">{{ followCounter.following
                                                }}</span>
                                                <br> Following
                                            </v-col>
                                        </v-row>
                            ...
                            <v-row class="mt-7">
                                <v-card-text>
                                    <v-card-title class="text-h5 text-center font-weight-bold">@{{ user.username }}</v-card-title>
                                </v-card-text>
                            </v-row>
                            <!-- Follow and unfollow buttons -->
                            <v-row v-if="showFollowButton">
                                <v-col cols align="center">
                                    <v-btn color="success" v-if="unfollowBtn" @click="followUser">
                                        Follow
                                    </v-btn>
                                </v-col>
                                <v-col cols align="center">
                                    <v-btn color="error" v-if="followBtn" @click="unfollowUser">
                                        Unfollow
                                    </v-btn>
                                </v-col>
                            </v-row>
                    ...
                    <!-- Notebooks container -->
                    <v-container>
                        <h3 class="text-h5 mt-4 mb-2">Notebooks this user owns:</h3>
                        <v-row v-if="ifNotebooks">
                            <v-col align="center" class="fill-height my-auto">
                                <h2 class="text-h6">No notebooks were found</h2>
                            </v-col>
                        </v-row>
                        <v-row v-else>
                            <v-col cols="12" md="4" v-for="(notebook, i) in userNotebooks" :key="i">
                                <v-card>
                                    <v-card-title class="text-h6 font-weight-bold">{{ notebook.title }}</v-card-title>
                                    <v-card-subtitle>Autor: {{ notebook.user.username }}</v-card-subtitle>
                                    <v-card-text>{{ notebook.description }}</v-card-text>
                                    <v-card-actions>
                                        <v-btn @click="readNotebookViewRedirection(notebook._id)" color="primary">
                                            Read
                                        </v-btn>
                                    </v-card-actions>
                                </v-card>
                ...
                <!-- Follows Dialogs -->
                <v-dialog v-model="followingDialog" transition="dialog-top-transition" max-width="500" max-height="400px">
                        ...
                        <v-card-text>
                            <v-row v-for="(follow, i) in follows" :key="i">
                                ...
                                <v-col cols="6" align="center" class="fill-height my-auto">
                                    <span class="font-weight-bold" @click="followRedirection(follow.followed._id)">{{
                                        follow.followed.username }}</span>
                                </v-col>
                            </v-row>
                        </v-card-text>
                    </v-card>
                </v-dialog>
                <v-dialog v-model="followersDialog" transition="dialog-top-transition" max-width="500" max-height="400px">
                    <v-card :title="dialogTitle" :subtitle="dialogSubtitle">
                        <v-card-text>
                            <v-row v-for="(follow, i) in follows" :key="i">
                                <v-col cols="6" align="center">
                                ...
                                <v-col cols="6" align="center" class="fill-height my-auto">
                                    <span class="font-weight-bold" @click="followRedirection(follow.user._id)">{{
